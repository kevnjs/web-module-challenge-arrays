...
function addFlavor(array, flavor){
  const newFlavor = array.unshift(flavor);
  return array;
}
...
function getAverageWordLength(array){
  ...
  let sum;
  for (let i = 0; i < array.length; i++) {
    wordCount.push(array[i].split(' ').length);
    }
   sum = wordCount.reduce((accumulator, currentValue) => {return accumulator + currentValue}, 0);
   return sum / wordCount.length;
  }
